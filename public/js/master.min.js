...
if($("#feed").length){$("#count-bar").load(' #navcount');$("#feed").prepend($response)}if($("#notweetmessage").length){$("#notweetmessage").hide()}
...
window.setInterval(function(){if(__feedcurrentid!=null){loadFeed(null,__feedcurrentid)}},15000);$(".tweet-alert").click(function(){$(".tweet-alert").hide();$("#newcount").text(0);$($newtweetbuffer).hide().prependTo("#feed").fadeIn("slow");$newtweetbuffer=" "});function redirectToLogin(){window.location="http://chirp.io/login"}
...
$('#navbar-search').keyup(function(){if($(this).val()!=''){$('#search-page').attr('href','/search/'+$(this).val());$('#navsearch').attr('action','/search/'+$(this).val());var criteria=$(this).val();$('#search-results-dropdown').show();$.ajax({url:'/search',type:'GET',data:{'q':$(this).val()},success:function(data){$('.search-item').remove();console.log(data);$('#search-results-dropdown').append("<li class='row search-item'><div class='col-xs-12'><ul class='list-unstyled'><li><h6>Tags</h6></li></ul></div></li>");if(data.tags.length!=0){for(var i=0;i<data.tags.length;i++){$element="<li class='row search-item'><div class='col-xs-12 highlight'><a href='/tag/"+data.tags[i].tag+"/tweets'><ul class='list-unstyled'><li><h6>#"+data.tags[i].tag+"</h6></li></ul></a></div></li>";$('#search-results-dropdown').append($element)}
...
else{$('#search-results-dropdown').append("<li class='row search-item'><div class='col-xs-12 text-center'>No Tags Found</div></li>")}
$('#search-results-dropdown').append("<li class='row search-item'><div class='col-xs-12'><ul class='list-unstyled'><li><h6>Users</h6></li></ul></div></li>");if(data.users.length!=0){for(var i=0;i<data.users.length;i++){$element="<li class='row search-item'><div class='col-xs-2'><img class='img-responsive img-circle' src='/avatars/"+data.users[i].profile_image+"' alt=''></div><div class='col-xs-10'><a href='/"+data.users[i].username+"'><ul class='list-unstyled'><li><h6>"+data.users[i].name+"</h6></li><li>@"+data.users[i].username+"</li></ul></a></div></li>";$('#search-results-dropdown').append($element)}
...
else{$('#search-results-dropdown').append("<li class='row search-item'><div class='col-xs-12 text-center'>No Users Found</div></li>")}},error:function(xhr){console.log(xhr)}});return!1}
...
exports.EmojiConvertor=emoji}else if(typeof define==='function'&&define.amd){define(function(){return emoji})}else{root.EmojiConvertor=emoji}}).call(function(){return this||(typeof window!=='undefined'?window:global)}());(function($){"use strict";var instance,proto;function UserScrollDisabler($container,options){this.opts=$.extend({handleWheel:!0,handleScrollbar:!0,handleKeys:!0,scrollEventKeys:[32,33,34,35,36,37,38,39,40]},options);this.$container=$container;this.$document=$(document);this.lockToScrollPos=[0,0];this.disable()}
...
else if(instance&&instance[method]){instance[method].call(instance)}};window.UserScrollDisabler=UserScrollDisabler})(jQuery);(function(root,factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else if(typeof exports==='object'){module.exports=factory(require('jquery'))}else{root.lightbox=factory(root.jQuery)}}(this,function($){function Lightbox(options){this.album=[];this.currentImageIndex=void 0;this.init();this.options=$.extend({},this.constructor.defaults);this.option(options)}
